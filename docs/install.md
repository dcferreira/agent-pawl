...
§B.15). Top-level `guards:` is now parsed and validated (id required+unique, `match:` required and
RE2-compilable, `only_in:` required — see [validation.md](validation.md) rule 15), but not yet
enforced: there is no `PreToolUse` hook in this build to actually deny a matched command, and `pawl
run`'s banner says so. Top-level `invariants:`, and a step's `retry:` field, remain unimplemented:
declaring either is still a validation error, not a quietly-ignored field.
