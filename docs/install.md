...
either `install.sh` or `go install`. Each release's notes come from `CHANGELOG.md` (generated by
changie from `.changes/`) — see [docs/releasing.md](releasing.md) for how a release is cut.
